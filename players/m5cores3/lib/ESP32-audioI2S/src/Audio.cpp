...
bool Audio::ts_parsePacket(uint8_t* packet, uint8_t* packetStart, uint8_t* packetLength) {
    ...
    if (PID == 0) {
        ...
        *packetLength = 0;
        ...
        // WAVEHOPPER PATCH (see WAVEHOPPER-PATCHES.md): AFC=3 with an adaptation
        // field that fills the whole payload area (posOfPacketStart >= 188)
        // carries no PES data — Livepeer emits these on the audio PID. Stock
        // 3.4.6 reads packet[188..] out of bounds, mistakes the garbage for a
        // missing PES start code, and aborts the stream. Treat as no-payload.
...
#ifdef WH_TS_DIAG
        {
            uint8_t pos = (AFL >= 0) ? 5 + AFL : 4;
            AUDIO_LOG_ERROR("WH: PES not found. PID=%d pidOfAAC=%d PUSI=%d AFC=%d AFL=%d "
                            "PES_DataLength=%d pos=%d bytes=%02X %02X %02X %02X %02X %02X",
                            PID, m_tspp.pidOfAAC, PUSI, AFC, AFL, m_tspp.PES_DataLength, pos,
                            packet[pos], packet[pos+1], packet[pos+2], packet[pos+3],
                            packet[pos+4], packet[pos+5]);
        }
#endif
...
#ifdef WH_TS_DIAG
                    AUDIO_LOG_ERROR("WH: PMT streamType=0x%02X elementaryPID=%d", streamType, elementaryPID);
#endif
