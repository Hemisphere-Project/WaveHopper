...
- **"Coming Soon" splash with universal-200.** A parked site can return HTTP 200 for *every* path because the static splash is the catch-all. If `/live`, `/listen`, `/player`, `/embed` all redirect to `/` and the title is `Coming Soon`, stop digging — the station hasn't launched (or has been pulled). Mark it `broken` and check back later. VIZI Radio fit this pattern: 570KB of HTML that was almost entirely a base64 favicon, with no `<audio>`/`<iframe>` and no stream-related strings anywhere in the markup.
