...
- **Livepeer audio-only shortcut.** Some Livepeer HLS masters accept `?video=false` and return an audio-only playlist (The Lot Radio dropped from a multi-rendition A/V ladder to a single ~193 kbps AAC stream). Keep the canonical station URL as the plain master in station metadata, but remember the player can append `video=false` at playback time when bandwidth matters.
...
- **AzuraCast (`<azuracast-host>/listen/<shortcode>/radio.mp3`)** is an open-source self-hosted radio platform — a common alternative to Airtime.pro for indie stations. Standard mountpoint is `/listen/<shortcode>/radio.mp3` (also `/radio.aac`, `/hls/<shortcode>/live.m3u8`). REST nowplaying at `/api/nowplaying/<shortcode>`, SSE at `/api/live/nowplaying/sse?cf_connect={"subs":{"station:<shortcode>":{"recover":true}}}`. CORS reflects the requesting Origin (good for `<audio>` from the station's own site, may need a relay for cross-origin Web Audio). Wave Zero used this on `stream.ondezero.net` with shortcode `oz`.
- **`window.__STATION_CONFIG__` / `window.__INITIAL_*__` global blobs.** Custom server-rendered radio sites often dump the entire player config (stream URL, host, shortcode, nowplaying endpoint, schedule) as a single JSON blob assigned to a `window.*` global in `/config.js` or inline. One `grep -nE 'window\.__|streamUrl|listen/|nowplaying' /tmp/config.js` typically yields everything in one shot — including the now-playing endpoint, saving a separate /import-now-playing pass.
