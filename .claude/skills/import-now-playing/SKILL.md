...
- **Airtime `/api/live-info-v2` has two modes: track and livestream.** When `tracks.current.type = "track"` (auto-DJ), use `metadata.track_title` → `title` and `metadata.artist_name` → `subtitle` with the track-level `starts`/`ends`. When `type = "livestream"` (live DJ), `tracks.current.name` is empty — fall back to `shows.current.name` → `title` and `shows.next[0]` → `next`. Timestamps are in the station's local timezone (`station.timezone`) — convert to UTC. Names are HTML-entity-encoded with possible leading whitespace. Use 20 s TTL (safe for both modes). Cache key: `airtime-<station-id>`.
- **LYL GraphQL: use `calendar`, not `onair`.** `onair { title }` only returns a combined `"Artist - Show"` string with no timestamps. The `calendar` query returns the full broadcast day as `{ startAt duration title artists }` — all UTC ISO 8601, no conversion needed. Find the current slot by comparing `now()` against `startAt + parsed_duration`. Map: `artists` → `title` (host), `title` → `subtitle` (show name). `wh_http_get` is GET-only — use `curl_init` directly for the POST. Cache key: `lyl-graphql` (30 s, shared across future LYL stations).
- **Radiocult schedule endpoint is GET and works well.** Use `GET /api/station/{slug}/schedule?startDate=<now>&endDate=<now+3h>` — returns `{ "schedules": [...] }` sorted by start time with `startDateUtc`/`endDateUtc`/`title` all UTC ISO 8601. No separate artist endpoint: `artistIds` are UUIDs that return `{"success":false}` from the public API; Noods embeds the host in the show title (e.g. "w/ Izzy Twist"). Store the Radiocult station slug in `nowPlaying.slug` so the same fetcher works for any future Radiocult station. Cache key: `radiocult-<station-id>` (30 s).
- **AzuraCast `/api/nowplaying/<shortcode>` is the right pick over the SSE.** REST returns full track + show + next + listener counts in one call, with `played_at` / `cued_at` already in Unix UTC seconds (no tz conversion, unlike Airtime). Mapping: `now_playing.song.title` → `title`, `now_playing.song.artist` → `subtitle`. When `live.is_live` is true, `live.streamer_name` becomes the title and the song title (if present) becomes the subtitle. Empty artist? Strip the `[CATEGORY] - ` prefix from `now_playing.playlist` and use that as a soft fallback. Treat `is_online: false` as null. The `/api/live/nowplaying/sse?cf_connect=...` alternative needs a persistent connection that doesn't fit the file-cached PHP model — skip it. Cache key: `azuracast-<station-id>` (20 s, fits track-level cadence). Endpoint stored in `nowPlaying.endpoint`.
- **The Lot homepage embeds the current lineup in the React Flight payload.** No dedicated now-playing JSON/XHR endpoint was found, and `ffprobe` on the live HLS variant exposed audio/video only (no visible ID3/data stream), so use the homepage HTML instead of `hls-id3`. Map current event `summary` → `title`, `subtitle` → null, `start`/`end` → `starts`/`ends`, and the next event's `summary`/`start` to `next`. Cache key: `thelot-html` (30 s). Endpoint stored in `nowPlaying.endpoint`.
